...
// window.addEventListener('click', () => {
    ...
    let optionList = document.querySelector("#default_currency");
    optionList.innerHTML  = "";
    for (const key in currencyList) {
        // console.log(key);
        if (Object.hasOwnProperty.call(currencyList, key)) {
            // const element = object[key];
            let newCurrencyList = document.createElement("option");
            let node = document.createTextNode(`${key} (${currencyList[key].name}) -- [${currencyList[key].symbol}]`);
            newCurrencyList.appendChild(node);
            newCurrencyList.value = `${key}`;
            if(key == "USD"){
                newCurrencyList.selected = true;
            }
